...
async function main() {
  await yargsInstance
    .command(
      "record <config>",
      "Run probes from a config and save a fingerprint",
      (y) =>
        y
          .positional("config", {
      ...
      async (args) => {
        ...
        const fp = await buildFingerprint(cfg, args.cwd as string | undefined);
        ...
        console.log(
          `Fingerprint saved to ${path} (${fp.probes.length} probes)`
        );
      }
    )
    .command(
      ...
      async (args) => {
        ...
        const baseline = JSON.parse(
          readFileSync(args.baseline as string, "utf-8")
        );
        const cfg = loadConfig(resolve(args.config as string));
