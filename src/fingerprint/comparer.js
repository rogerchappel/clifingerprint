...
export function compareFingerprints(baseline, current) {
  ...
  for (const [name, baselineProbe] of baselineMap) {
    ...
    if (baselineProbe.expectedExitCode !== currentProbe.expectedExitCode)
      changes.push(
        `expected exit changed: ${baselineProbe.expectedExitCode} -> ${currentProbe.expectedExitCode}`,
      );
    if (baselineProbe.expectedExitMatched !== currentProbe.expectedExitMatched)
      changes.push(
        `expected exit match changed: ${baselineProbe.expectedExitMatched} -> ${currentProbe.expectedExitMatched}`,
      );
...
function compareFlags(baselineText, currentText) {
  ...
  for (const flag of baselineFlags) {
    if (!currentFlags.has(flag)) changes.push(`flag removed: ${flag}`);
  }
  for (const flag of currentFlags) {
    if (!baselineFlags.has(flag)) changes.push(`flag added: ${flag}`);
  }
...
function comparePackageMetadata(baselinePackage, currentPackage) {
  if (!baselinePackage && !currentPackage) return [];
  if (!baselinePackage) return ["package metadata added"];
  if (!currentPackage) return ["package metadata removed"];
