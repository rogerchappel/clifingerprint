...
export function compareFingerprints(
  ...
  for (const [name, baselineProbe] of baselineMap) {
    ...
    if (baselineProbe.exitCode !== currentProbe.exitCode) {
      changes.push(
        `exit code changed: ${baselineProbe.exitCode} -> ${currentProbe.exitCode}`
      );
    }
    if (baselineProbe.timedOut !== currentProbe.timedOut) {
      changes.push(
        `timeout changed: ${baselineProbe.timedOut} -> ${currentProbe.timedOut}`
      );
    }
    ...
    if (baselineProbe.skipped !== currentProbe.skipped) {
      changes.push(
        `skip changed: ${baselineProbe.skipped} -> ${currentProbe.skipped}`
      );
    }
